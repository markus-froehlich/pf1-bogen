...
export function CharacterSheet({ index, activeId, onSwitch, onNew, onDelete, onClose, lang, raceMap = {}, classMap = {}, player }) {
  const L = lang === 'de'
            ...
            <div key={entry.id} className={`nc-char-row ${active ? 'is-active' : ''}`}>
              ...
              {confirmId === entry.id && (
                <div className="nc-char-confirm">
                  ...
                  <button className="nc-btn nc-btn-danger" onClick={() => { onDelete(entry.id); setConfirmId(null) }}>{L ? 'Löschen' : 'Delete'}</button>
                </div>
              )}
            </div>
          )
