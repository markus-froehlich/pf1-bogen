...
export function CompanionsTab({ index, ownerId, onCreate, onOpen, lang }) {
  ...
  const [expanded, setExpanded] = useState(() => companions.length === 0)
  ...
  return (
    <div className="companions-tab">
      <button className="companions-intro" onClick={() => setExpanded(value => !value)} aria-expanded={expanded}>
        <div>
          <div className="companions-kicker">{L ? 'VERKNÜPFTE GEFÄHRTEN' : 'LINKED COMPANIONS'}</div>
          <h2>{L ? 'Tiergefährten' : 'Animal Companions'}</h2>
          {!expanded && <p>{L ? 'Auswahl und bestehende Gefährten anzeigen' : 'Show selection and existing companions'}</p>}
        </div>
        <span className="companions-header-end"><span className="companions-count">{companions.length}</span><span className="companions-toggle">{expanded ? '▾' : '▸'}</span></span>
      </button>
