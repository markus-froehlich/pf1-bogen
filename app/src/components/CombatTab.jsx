  ...
  const renderSection = (id, idx) => {
    ...
    if (id === 'combat') return (
      <section key="combat" className="ct-section">
        <SectionHead id="combat" label={L ? 'Kampfwerte' : 'Combat values'} summary={`I${combat.init} · KMB${combat.kmb} · KMV${combat.kmv}`} idx={idx} count={count} onMove={onMoveSection} collapsed={isCollapsed} onToggle={onToggleCollapse} />
        {!isCollapsed && <>
